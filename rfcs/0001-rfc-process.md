...
This RFC defines how ActiveVersion design changes are proposed, written, numbered, and advanced.

## Motivation

Generator names, destination-model configuration, audit storage modes, and optional trigger SQL are the public contract. Those choices occupy method names and create tables hosts already migrated. Numbered RFCs are the unit for a change that would move the gem version for every model that included the concerns. RFC 0002 records why translations, revisions, and audits live in one gem with explicit per-model wiring.
