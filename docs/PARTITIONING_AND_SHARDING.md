...
| **Source model with Rails composite PK** (`self.primary_key = [...]`) | **Not first-class yet** | Current revision/translation internals use a single `source_foreign_key` column (e.g. `post_id`). Audit internals use polymorphic `auditable_id`/`auditable_type`. |
...
- Use Rails composite PK (`self.primary_key = [...]`) on the **version table** when partitioned by PostgreSQL and you need PK to include partition key.
- Keep source model PK as a single column (`id`) for ActiveVersion relations/callbacks.
