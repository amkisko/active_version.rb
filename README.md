...
- **Audit schema is configured explicitly on destination audit models.**
  Storage mode and audit column mapping belong to the audit model/table contract (`configure_audit`, alias `active_version_audit`), not hidden global magic.
...
sql = post.audit_sql
Post.batch_insert_sql([post1, post2])
```
...
class Post < ApplicationRecord
  has_audits(
    ...
    auditable_id_method: :external_id, # optional: use custom identity value for auditable_id
  ...
  has_revisions(
    ...
    foreign_key_value: :external_id    # optional: source value used for FK writes/queries
  )
...
- `batch_insert_sql(records)` - Generate SQL for batch audits
